...
export function TasksArea({ screen, onScreenChange, space, members, userId, moduleState, moduleError, moduleBusy, isOwner, onModuleRetry, onModuleToggle, invitePanel, onMembersOpen, onSpaceSelectorOpen }: TasksAreaProps) {
  ...
  const readGate = useRef(createTaskReadGate());
  ...
  async function reloadTasks() {
    ...
    try {
      ...
      for (let start = 0; ;) {
        ...
        if (count === null) throw new Error('无法确认任务列表是否完整。');
        const batch = (data ?? []) as Task[];
        loaded.push(...batch);
        if (loaded.length >= count) break;
        if (batch.length === 0) throw new Error('任务列表读取不完整，请重试。');
        start += batch.length;
      }
      if (readGate.current.isActive() && requestGuard.current.isCurrent(currentRequest)) {
        setTasks(loaded);
        setLoading(false);
        setError('');
      }
  ...
  useEffect(() => {
    ...
    setSyncError('');
    if (!tasksEnabled) {
      readGate.current.deactivate();
      setCreating(false);
      setEditingTask(null);
      return;
    }
    readGate.current.activate();
    ...
    const channel = supabase
      ...
      .subscribe((status) => {
        ...
        if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') setSyncError('任务实时同步暂不可用，请检查连接。');
      });
  ...
  }, [space.id, tasksEnabled]);

  async function refreshAfterMutation() {
    await readGate.current.runIfActive(() => reloadTasks().catch(() => undefined));
  }

  async function changeStatus(task: Task, status: Task['status']) {
    if (!readGate.current.isActive() || !tasksEnabled || !canChangeTaskStatus(task, userId)) return;
    ...
    try {
      ...
      if (readGate.current.isActive()) setError(taskErrorMessage(statusError));
    } finally {
      if (readGate.current.isActive()) setBusyTaskId(null);
    }
  }

  if (visibleScreen === 'calendar') return null;

  const list = visibleScreen === 'completed' ? grouped.completed : grouped.open;
  ...
  return (
    <>
      ...
      <section className="flex-1 px-4 py-4 safe-bottom">
        {tasksEnabled && error && <div className="mb-4 rounded-lg bg-coral/10 p-4 text-sm text-coral" role="alert">{error}<button className="ml-3 font-semibold underline" type="button" onClick={() => void reloadTasks().catch(() => undefined)}>重试</button></div>}
        {tasksEnabled && syncError && <p className="mb-4 rounded-lg bg-coral/10 px-4 py-3 text-sm text-coral" role="alert">{syncError}</p>}
        ...
        {visibleScreen === 'hub' ? (
          <div className="space-y-4">
            ...
            <div className="rounded-lg bg-white px-4 py-4 shadow-sm">
              <h2 className="text-sm font-semibold text-ink/60">模块</h2>
              <div className="mt-2 flex min-h-11 items-center justify-between gap-3">
                <span className="font-semibold">任务</span>
                {moduleState === 'loading' ? <span className="text-sm text-ink/60">读取中</span> : moduleState === 'error' ? (
                  <button className="min-h-11 px-2 text-sm font-semibold text-teal" type="button" onClick={onModuleRetry}>重试</button>
                ) : isOwner ? (
                  <div className="flex shrink-0 items-center gap-2">
                    <span className="text-sm text-ink/60">{moduleState === 'enabled' ? '已开启' : '已关闭'}</span>
                    <button className="min-h-11 rounded-lg bg-mist px-4 text-sm font-semibold text-teal disabled:opacity-60" type="button" disabled={moduleBusy} onClick={onModuleToggle} aria-label={`${moduleState === 'enabled' ? '关闭' : '开启'}任务模块`}>
                      {moduleBusy ? '处理中' : moduleState === 'enabled' ? '关闭' : '开启'}
                    </button>
                  </div>
                ) : <span className="text-sm text-ink/60">{moduleState === 'enabled' ? '已开启' : '已关闭'}</span>}
              </div>
            ...
            {tasksEnabled && <button className="flex min-h-16 w-full items-center justify-between gap-3 rounded-lg bg-white px-4 text-left shadow-sm" type="button" onClick={() => onScreenChange('tasks')} aria-label="进入任务">
              <span className="font-semibold">任务</span>
          ...
          <div className="space-y-4">
            {visibleScreen === 'tasks' && <h2 className="text-sm font-semibold text-ink/60">待完成 · {grouped.open.length}</h2>}
            {loading ? (
              <p className="rounded-lg bg-white px-4 py-5 text-sm text-ink/60 shadow-sm">正在读取任务…</p>
            ) : list.length === 0 && !error ? (
              <p className="rounded-lg bg-white px-4 py-5 text-sm text-ink/60 shadow-sm">{visibleScreen === 'completed' ? '暂无已完成事项' : '暂无待完成事项'}</p>
            ) : (
              <ul className="divide-y divide-ink/10 overflow-hidden rounded-lg bg-white shadow-sm">
                {list.map((task) => (
                  <li key={task.id} className="flex items-start gap-2 px-3 py-2">
                    {visibleScreen === 'tasks' && canChangeTaskStatus(task, userId) ? (
                    ...
                    {visibleScreen === 'completed' && canChangeTaskStatus(task, userId) && (
      ...
      {tasksEnabled && (creating || editingTask) && (
        <TaskSheet
          ...
          onClose={() => { if (readGate.current.isActive()) { setCreating(false); setEditingTask(null); } }}
        />
      )}
    </>
  );
