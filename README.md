...
- v0.1.14.1 Navigation Persistence 已在 Production 完成用户真实账号刷新验收并 `CLOSED / PASS`。v0.1.15 Shared Lists Slice 1 DB foundation `CLOSED / PASS`；Slice 2 本地自动验证与用户桌面真实账号验收已通过，双账号 filtered Realtime DELETE + RLS/canonical reread 已通过；功能中心会话可用性与 Tasks/Review/Lists 可见性复核通过。Vercel 前端部署待核验，移动端/PWA 验收留待部署后；Slice 2 尚未关闭，Slices 3–4 未开始。详见 [v0.1.15 Shared Lists Specification](./docs/v0.1.15_SHARED_LISTS_SPEC.md)。
...
- Shared Lists Slice 2 前端部署尚未验证；Slice 1 Production DB foundation 已通过，v0.1.15 产品与技术边界见 [canonical specification](./docs/v0.1.15_SHARED_LISTS_SPEC.md)。Task Reminder、recurring Tasks、纪念日、聊天、相册
