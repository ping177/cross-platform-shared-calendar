...
- 当前开发/部署版本：`v0.1.10 — Personal Space + Multi-space + Module Enablement Foundation`，其中 Slice 2 frontend 已部署、authenticated acceptance 待用户执行。`v0.1.9 — Shared Tasks MVP` 仍是最新 `CLOSED / PASS` 的已验收 capability：其 backend foundation 与 status-ownership correction 已应用至 Production，Desktop A/B 与 iPhone smoke 均 PASS。Task 规范见 [v0.1.9 Shared Tasks Spec](./docs/v0.1.9_SHARED_TASKS_SPEC.md)；长期导航、Space 模型和方向性路线图见 [Shared Life Architecture Freeze](./docs/SHARED_LIFE_ARCHITECTURE.md)。
- `v0.1.8 — Mobile Push Reminder` 仍为 `CLOSED / PASS`：Push Infrastructure、ordinary Reminder delivery、Slice 3 Recurrence Reminder Integration 与 final cross-platform acceptance 已完成。`v0.1.10 — Personal Space + Multi-space + Module Enablement Foundation` 的 Slice 1 backend 已通过 Production rollout/postflight；Slice 2 frontend 已部署并完成未登录/静态验证，authenticated Production acceptance 尚未开始；Slice 3 未开始。v0.1.9 仍是最新经 authenticated acceptance 的 capability，但 Production 当前入口已是 Slice 2 frontend。
