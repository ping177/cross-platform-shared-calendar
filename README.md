...
- 当前开发/部署版本：`v0.1.10 — Personal Space + Multi-space + Module Enablement Foundation`。Slice 1 backend 为 `PRODUCTION BACKEND ROLLOUT PASS`；Slice 2 为 `CLOSED / PASS`，包含 Production frontend rollout 与用户执行的 A/B authenticated acceptance；Slice 3 frontend 为 `DEPLOYED / UNAUTHENTICATED VERIFIED`，authenticated Production acceptance 尚未开始。`v0.1.9 — Shared Tasks MVP` 的独立历史验收仍为 `CLOSED / PASS`。Task 规范见 [v0.1.9 Shared Tasks Spec](./docs/v0.1.9_SHARED_TASKS_SPEC.md)；长期导航、Space 模型和方向性路线图见 [Shared Life Architecture Freeze](./docs/SHARED_LIFE_ARCHITECTURE.md)。
- `v0.1.8 — Mobile Push Reminder` 仍为 `CLOSED / PASS`：Push Infrastructure、ordinary Reminder delivery、Slice 3 Recurrence Reminder Integration 与 final cross-platform acceptance 已完成。v0.1.10 的 Personal Space、多 Space 选择、Event/Task 隔离、A/B Realtime、320px 与 iPhone/PWA 验收已通过；表单打开时切换 Space 与 Shared ↔ Shared 按本轮记录为 N/A，不是失败。整个 v0.1.10 尚未关闭，Next Action 是用户执行 Slice 3 authenticated Production module-toggle acceptance。
