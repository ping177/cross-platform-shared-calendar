...
v0.1.11 is `IN PROGRESS / DESIGN FROZEN`. Slice 1 Navigation Foundation is `CLOSED / PASS` and deployed. Slice 2 Aggregate Calendar is `CLOSED / PASS`: implementation, automated verification, local authenticated functional acceptance, Production frontend rollout and bounded Production smoke passed. Implementation commit `0505091` is on `origin/main`; Slice 2 required no backend rollout. Dedicated Slice 2 iPhone Safari, installed PWA and final 320px device checks are `DEFERRED / NOT RUN`; second Shared Space coverage is `N/A / NOT RUN`. Slices 3–4 have not started. Home remains reserved for Slice 3; v0.1.10 remains `CLOSED / PASS`.
...
- v0.1.11 — Navigation + Aggregation Experience（IN PROGRESS / DESIGN FROZEN；Slice 1 CLOSED / PASS；Slice 2 CLOSED / PASS；Slices 3–4 NOT STARTED）
...
Begin v0.1.11 Slice 3 — Home Aggregation with a read-only repository/design review against the frozen aggregation architecture before implementation. Slice 2 is `CLOSED / PASS`; device checks 11–13 remain `DEFERRED / NOT RUN`, and check 14 is `N/A / NOT RUN`.
...
- v0.1.11 canonical scope is `docs/v0.1.11_NAVIGATION_AGGREGATION_SPEC.md`. Slice 1 and Slice 2 are `CLOSED / PASS`; user-run bounded Production smoke passed for Slice 2, which required no backend/schema/RPC change. Calendar all/one-Space filtering and active-view Event aggregation are in Slice 2. Home and Task aggregation remain Slice 3; global Event/Task create remains Slice 4. Dedicated Slice 2 device checks remain deferred as recorded above.
...
v0.1.10 remains `CLOSED / PASS`. Production includes v0.1.11 Slice 1 Navigation Foundation and Slice 2 Aggregate Calendar. Slice 2 is `CLOSED / PASS` after local automated checks, user-run authenticated acceptance, Production frontend rollout and bounded Production smoke; no backend rollout was required. Dedicated Slice 2 iPhone Safari, installed PWA and final 320px checks are `DEFERRED / NOT RUN`; second Shared Space coverage is `N/A / NOT RUN`. Overall v0.1.11 remains `IN PROGRESS`; Slice 3 — Home Aggregation is `NOT STARTED`. Next: perform a read-only repository/design review against the frozen aggregation architecture before Slice 3 implementation. Home begins only with all-Space aggregation semantics.
