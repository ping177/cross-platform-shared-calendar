...
v0.1.8.2 (Reminder Persistence + Ordinary Event Delivery — CLOSED / PASS; Slice C closed)

## Current status

Calendar Core 与 Recurring Events 已完成并通过 Production 验收。`v0.1.8.1 — Push Infrastructure Foundation` 保持 `CLOSED / PASS — Android final acceptance deferred`。`v0.1.8.2` Slice A、Slice B 与 Slice C 现均为 `CLOSED / PASS`。P3A C1 Production foundation、P3B `send-reminders` 手工 Production E2E 与 P3C automatic scheduler E2E 全部 PASS。Production 中 `send-reminders` 为 ACTIVE v1 且 `verify_jwt=false`，Vault `REMINDER_CRON_SECRET` 仅确认名称且恰有一项，`pg_cron` / `pg_net` 已启用，并且恰有一个 `send-reminders-every-minute` once-per-minute scheduler 使用执行时 Vault lookup。automatic no-due、iPhone/Mac automatic real Reminder、ledger/idempotency 与持续 scheduled-run postflight 全部 PASS。整体 `v0.1.8` 仍保持 OPEN：Slice 3 recurrence Reminder integration 与最终 cross-platform/Android acceptance 尚未开始。

## Latest completed

Completed the bounded P3C scheduler activation and automatic Production acceptance on canonical project `ximazjhxvmktpcdbypka`. `pg_cron` and `pg_net` are enabled in their canonical schemas; exactly one active once-per-minute job targets Production `send-reminders`, stores only the approved Vault lookup command, and contains neither plaintext secret nor service-role bearer. Automatic no-due runs completed with zero work. One disposable personal timed Event was discovered without any manual invocation and automatically delivered to both active subscriptions; iPhone and Mac notifications both passed. Ledger postflight has exactly two finalized `sent` rows for the test due, two distinct subscriptions, zero claimed/failed rows, zero duplicate identities, and zero unexpected subscription disablement. Four repeated due-window runs rejected the already-claimed tasks without extra sends. A bounded 120-run postflight showed 120 successful Cron executions and HTTP 200 responses, no duplicate minute execution, no timeout/error, and no sensitive diagnostics. Slice C is `CLOSED / PASS`; the scheduler remains active with unschedule-first as the canonical rollback.
...
Notes: 现有公网版本继续服务；`v0.1.8.2` Slice A/B/C 均已完成 Production acceptance。`send-test-push` 保持 ACTIVE v4 reviewed-equivalent；`send-reminders` 保持 ACTIVE v1 / `verify_jwt=false`。Vault Reminder secret 按名称确认恰有一项，`pg_cron` / `pg_net` 已启用，且恰有一个 `send-reminders-every-minute` scheduler 每分钟自动调用 Production Function。P3C no-due 与 automatic real-device E2E 均 PASS；unschedule-first 仍是 scheduler failure containment。Slice 3 recurrence Reminder 与 Android final Push acceptance 尚未开始。
...
Separately authorize the already-frozen Slice 3 Recurrence Reminder Integration: canonical dynamic occurrence projection, reminder/timezone inheritance, override/delete/split/current-and-future semantics, recurring idempotency, and timezone/DST/no-duplicate coverage. Do not begin final cross-platform/Android acceptance or invent a new version without separate authorization.
...
- v0.1.8 uses one active once-per-minute Supabase Cron + Edge Function sender. Slice 2 ordinary Event delivery is closed; Slice 3 still must dynamically project recurring occurrences through canonical recurrence/exception semantics without materializing a long horizon.
...
v0.1.8.2 is `CLOSED / PASS`; Slice A/B/C are closed. P3A C1, P3B manual `send-reminders` E2E, and P3C automatic scheduler E2E all passed. Production has `send-reminders` ACTIVE v1 / `verify_jwt=false`, one Vault `REMINDER_CRON_SECRET` by name, enabled `pg_cron` / `pg_net`, and exactly one active `send-reminders-every-minute` scheduler using an execution-time Vault lookup. Automatic no-due verification and one real personal timed Reminder delivered automatically to both iPhone and Mac; the two ledger rows are `sent`, repeated scheduled invocations did not duplicate, and no subscription was disabled. Unschedule-first remains the rollback rule. Overall v0.1.8 remains open. Next: separately authorize the already-frozen Slice 3 Recurrence Reminder Integration; final Android/cross-platform acceptance remains later and separate.
