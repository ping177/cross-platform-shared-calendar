...
v0.1.11 为 `IN PROGRESS / DESIGN FROZEN`。Slice 1–3 已 `CLOSED / PASS` 并进入 Production。Slice 4 本地实现与真实账号 Desktop 验收已完成；Production iPhone Safari / installed PWA 验收仍待完成，并须针对已部署的 Slice 4 前端执行。Slice 4 和 v0.1.11 均未关闭。Slice 3 独立日程/任务区块错误与重试模拟仍为 `NOT RUN / DIFFICULT TO SIMULATE SAFELY`；第二个共享空间为 `N/A / NOT RUN`。Slice 2 专项设备检查仍为 `DEFERRED / NOT RUN`。v0.1.10 保持 `CLOSED / PASS`。

## Latest completed

Slice 4 本地实现经 UX 调整：首页“近期日程 + / 需要处理的任务 +”分别直接进入对应创建流程；首页标题按钮及类型选择面板已移除。用户报告真实账号 Desktop 验收通过：Personal/Shared 创建与归属、安全确认取消、关闭重开、防重复提交、模块关闭保护及两条现有本地创建入口均通过。默认目标独立设为个人空间；现有 EventSheet/TaskSheet 保留目标选择、表单内第二次确认、保存前身份/成员/模块重读、同步防重复提交和成功后主动刷新首页。Production iPhone Safari / installed PWA 最终验收仍待完成，须针对已部署的 Slice 4 前端进行；Slice 4 未关闭。
...
Notes: Vercel is the configured Production provider. Slice 1–3 Production acceptance remains `PASS`; Slice 4 local implementation and user-reported Desktop authenticated acceptance are complete. Remaining user acceptance is iPhone Safari, installed PWA, and necessary create/save/layout regression checks against the Slice 4 frontend served from Production. No Slice 3/4 backend rollout occurred, and the backend remains unchanged from v0.1.10. Dedicated Slice 2 iPhone Safari, installed PWA and final 320px device checks remain `DEFERRED / NOT RUN`; second Shared Space coverage is `N/A / NOT RUN`. Slice 3 independent Event/Task section error/retry simulation remains `NOT RUN / DIFFICULT TO SIMULATE SAFELY`. `space_modules` still has no Realtime publication, so another member may need refresh/re-entry after a remote Tasks toggle; database policy blocks disabled writes. v0.1.10 remains `CLOSED / PASS`. `send-test-push` remains ACTIVE v4 reviewed-equivalent; `send-reminders` remains ACTIVE v2 / `verify_jwt=false`. Vault/secret/Cron were not changed.
...
- v0.1.11 — Navigation + Aggregation Experience（IN PROGRESS / DESIGN FROZEN；Slice 1 CLOSED / PASS；Slice 2 CLOSED / PASS；Slice 3 CLOSED / PASS；Slice 4 LOCAL IMPLEMENTATION + DESKTOP AUTH ACCEPTANCE COMPLETE / PRODUCTION IPHONE/PWA ACCEPTANCE PENDING）
...
Complete Production iPhone Safari and installed PWA acceptance for v0.1.11 Slice 4 after the authorized `main` push and Vercel deployment. Verify the two Home section entries, necessary Production create/save behavior, narrow layout and keyboard/safe-area reachability. Do not use local 5175/LAN for final device acceptance. Keep Slice 4 and v0.1.11 open.
...
- v0.1.11 canonical scope is `docs/v0.1.11_NAVIGATION_AGGREGATION_SPEC.md`. Slices 1–3 are `CLOSED / PASS` in Production. Slice 4 local implementation and user-reported authenticated Desktop acceptance are complete; 首页区块日程/任务快捷入口及目标安全规则已冻结。Production iPhone Safari and installed PWA acceptance remain pending deployment; Slice 4 remains open. Slice 3 independent Event/Task section error/retry is `NOT RUN / DIFFICULT TO SIMULATE SAFELY`; second Shared Space is `N/A / NOT RUN`. Dedicated Slice 2 device checks remain deferred as recorded above.
...
v0.1.10 remains `CLOSED / PASS`. Production includes v0.1.11 Slices 1–3, all `CLOSED / PASS`. Slice 4：首页“近期日程 + / 需要处理的任务 +”分别直达 canonical Sheets；Personal 默认、目标切换、二次确认和保存前安全检查均保留。用户报告本地真实账号 Desktop 验收通过；Production iPhone Safari / installed PWA 验收须等部署后完成。Do not mark Slice 4 or v0.1.11 closed. Slice 3 independent section error/retry is `NOT RUN / DIFFICULT TO SIMULATE SAFELY`; second Shared Space is `N/A / NOT RUN`.
