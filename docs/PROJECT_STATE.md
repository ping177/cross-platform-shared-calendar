...
v0.1.11 is `IN PROGRESS / DESIGN FROZEN`. Slice 1 Navigation Foundation, Slice 2 Aggregate Calendar, and Slice 3 Home Aggregation are `CLOSED / PASS`. Slice 3 passed user-run desktop, Vercel Production, iPhone Safari, and installed PWA acceptance; Production includes Slices 1–3. Independent Event/Task section error/retry simulation remains `NOT RUN / DIFFICULT TO SIMULATE SAFELY`; second Shared Space coverage is `N/A / NOT RUN` (no extra Space created for acceptance). Slice 4 Global Create Safety has not started. Dedicated Slice 2 device checks remain `DEFERRED / NOT RUN`. v0.1.10 remains `CLOSED / PASS`.
...
- v0.1.11 — Navigation + Aggregation Experience（IN PROGRESS / DESIGN FROZEN；Slice 1 CLOSED / PASS；Slice 2 CLOSED / PASS；Slice 3 CLOSED / PASS；Slice 4 NOT STARTED）
...
Begin the v0.1.11 Slice 4 — Global Create Safety repo/design review and planning for the frozen `Global + → 日程 / 任务` flow. Slice 4 implementation has not started. Slice 3 independent Event/Task section error/retry simulation remains `NOT RUN / DIFFICULT TO SIMULATE SAFELY`; second Shared Space is `N/A / NOT RUN`.
...
- v0.1.11 canonical scope is `docs/v0.1.11_NAVIGATION_AGGREGATION_SPEC.md`. Slices 1–3 are `CLOSED / PASS`; Production includes Slice 3, and user-run desktop, Vercel Production, iPhone Safari, and installed PWA acceptance passed. Independent Event/Task section error/retry is `NOT RUN / DIFFICULT TO SIMULATE SAFELY`; second Shared Space is `N/A / NOT RUN`. Global Event/Task create remains Slice 4. Dedicated Slice 2 device checks remain deferred as recorded above.
...
v0.1.10 remains `CLOSED / PASS`. Production includes v0.1.11 Slices 1–3, all `CLOSED / PASS`. Slice 3 Home Aggregation passed user-run desktop, Vercel Production, iPhone Safari, and installed PWA acceptance, including Home Event/Task flows and the Space → Tasks regression retest. Independent section error/retry is `NOT RUN / DIFFICULT TO SIMULATE SAFELY`; second Shared Space is `N/A / NOT RUN`. Slice 4 remains `NOT STARTED`.
