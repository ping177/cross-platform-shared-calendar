...
## v0.1.10 Slice 2 — Selected / Current Space Vertical Flow (LOCAL VERIFIED; REVIEW PENDING)
...
- Authenticated Production acceptance: `NOT STARTED`. Production frontend rollout: `NOT STARTED`. The new frontend calls `ensure_personal_space()` on its first successful authenticated bootstrap; do not run it with a real Production account until the rollout strategy and user-run acceptance are authorized. The v0.1.9 Vercel frontend remains the latest accepted user-visible Production capability. No Personal Space backfill is authorized.
...
- Complete 320px mobile and supported PWA smoke only after the environment alignment gate and rollout strategy are approved. The user, not Codex, performs real OTP, A/B authenticated browser, and device acceptance.
...
- Old frontend compatibility: current users still have only one Shared membership; no Personal Space was created, old create/join RPC signatures and Event/Task calls remain compatible, and Tasks are enabled for all existing Spaces. Real authenticated v0.1.10 frontend acceptance has not started. Production Task count was 0 at rollout, so disabled-history preservation/re-enable behavior has only local regression evidence; no artificial Production Task was created. Next: Slice 2 selected/current Space flow.
