...
## v0.1.10 Slice 1 — Data / Permission Foundation (COMPLETE / LOCAL VERIFIED)
...
- Production preflight, patch application, postflight, and real authenticated acceptance have **not started**. The next, separately gated rollout begins with read-only verification of the actual v0.1.9 `spaces`/`space_members`/`events`/`tasks` schema, `one_space_per_user_idx`, member and Task RLS, create/join/rotate RPC definitions, existing data identities/counts, PostgreSQL version, and absence of partial v0.1.10 objects. Apply only `supabase/patches/2026-09-23-v0.1.10-multispace-foundation.sql` after that gate; postflight must verify old Spaces are `shared`, all old Spaces have `tasks=true`, only the partial Personal unique index remains, new RPC ACLs and guards match source, old row counts/identities are stable, and no Personal Space was created. Preserve the old frontend until the selected-Space frontend and authenticated acceptance are ready.
