...
- **Slice 2 implementation status:** `v0.1.8.2 — Reminder Persistence + Ordinary Event Delivery` architecture / semantics remain frozen. Slice A is closed and Slice B is `CLOSED / PASS` after local verification, Production database migration, frontend rollout, and Production human acceptance. Slice C1 ledger + atomic claim is implemented locally and under review; sender / Edge Function / Cron have not started, C1 is not in Production, and Slice C overall is not closed. This freeze supersedes the earlier Option A `events.reminder_offset_minutes` decision.
