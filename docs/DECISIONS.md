...
- **Slice 2 implementation status:** `v0.1.8.2 — Reminder Persistence + Ordinary Event Delivery` architecture / semantics remain frozen. Slice A and Slice B are closed. Slice C1 ledger + atomic claim completed implementation, verification, human review, and push but is not in Production. Slice C2 Phase 1 shared sender extraction is `CLOSED / PASS` after automated verification and Desktop/iPhone real-device regression; only `send-test-push` was deployed. Repeated Desktop banners were classified as pre-existing fixed-tag behavior and non-blocking. `send-reminders`, candidate scan, Cron, secrets, and Production rollout have not started, and Slice C overall is not closed. This freeze supersedes the earlier Option A `events.reminder_offset_minutes` decision.
