...
- **Implementation status:** `v0.1.8.2 — Reminder Persistence + Ordinary Event Delivery` Slice A/B/C and Slice 3 Recurrence Reminder Integration are `CLOSED / PASS` in Production. The reviewed long-duration correction changes only the Reminder projection adapter and leaves the canonical recurrence engine unchanged. The Slice 3 DB patch, `send-reminders` v2, frontend rollout, scheduler health, ordinary regression, recurring override/delete/split behavior, and real iPhone/Mac automatic Push passed Production acceptance. Overall v0.1.8 remains OPEN only for final cross-platform / Android acceptance. This freeze supersedes the earlier Option A `events.reminder_offset_minutes` decision.
