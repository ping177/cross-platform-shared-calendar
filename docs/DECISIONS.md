...
- **Slice 2 implementation status:** `v0.1.8.2 — Reminder Persistence + Ordinary Event Delivery` architecture / semantics remain frozen. Slice A is closed. Slice B persistence + Event mutation/UI, local ordered-upgrade, pgTAP/database regressions, Node/Vite/Deno verification, and Production database migration are complete. Frontend rollout and Production human acceptance remain pending; Slice B is not yet closed. Slice C ledger, sender, and Cron have not started. This freeze supersedes the earlier Option A `events.reminder_offset_minutes` decision.
...
- **Delivery ledger:** Slice 2 creates rows only for currently due, eligible ordinary Event/subscription pairs after atomic current-state revalidation. The minimal lifecycle is `claimed` / `sent` / `failed`; there is no long-lived pending queue, mutation-time ledger cancellation, processing recovery, retry scheduler, or generic background-job abstraction.
- **Idempotency:** ordinary Slice 2 deliveries are unique by `(event_id, subscription_id, due_at)`. `recipient_user_id` is audit context rather than part of uniqueness. Recurring occurrence identity is deliberately deferred until Slice 3 establishes its authoritative occurrence model; Slice 2 does not add `occurrence_key`. Provider errors, including ambiguous timeouts, record `failed` and are not automatically resent; the roughly ten-minute grace covers an eligible due that had not yet been claimed because Cron/Edge execution was late.
