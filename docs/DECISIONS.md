...
- **Implementation status:** `v0.1.8.2 — Reminder Persistence + Ordinary Event Delivery` Slice A/B/C is `CLOSED / PASS` in Production. Slice 3 Recurrence Reminder Integration is implemented and verified locally, and final human/code review is `PASS`; it remains not deployed. The reviewed long-duration correction changes only the Reminder projection adapter and leaves the canonical recurrence engine unchanged. Production ordinary delivery, scheduler, Vault, Cron, secrets, and deployed Functions were not changed by Slice 3. This freeze supersedes the earlier Option A `events.reminder_offset_minutes` decision.
...
- **Recurrence:** Slice 2 Production continues ordinary non-recurring delivery. The local Slice 3 implementation dynamically projects due occurrences through the existing runtime-neutral recurrence engine using source Event + rule + cutoff + exceptions. Its timezone/calendar-bounded window covers every frozen Reminder kind and grace period, including moved-in overrides, without a second recurrence/DST algorithm or occurrence materialization.
