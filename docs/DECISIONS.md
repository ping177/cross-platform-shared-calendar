...
- **Slice 2 freeze:** `v0.1.8.2 — Reminder Persistence + Ordinary Event Delivery` architecture / semantics are frozen. Slice A's shared timezone primitives and due calculator are implemented and cross-runtime verified locally, awaiting final human review plus commit/push governance closeout; persistence, UI, ledger, sender, and Cron have not started. This freeze supersedes the earlier Option A `events.reminder_offset_minutes` decision.
