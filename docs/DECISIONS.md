...
- v0.1.14 的产品语义于 2026-09-25 完成 docs-only 设计冻结；canonical 数据、权限、UI、slices 和验收契约见 [v0.1.14 规格](./v0.1.14_STRUCTURED_CHECKIN_SPEC.md)。Backend/date chronology、Slice 2–4 frontend、acceptance fixes、Production deployment/public smoke、用户报告的 Production authenticated acceptance 与最终 exact-ID fixture cleanup 均已通过；v0.1.14 现为 `CLOSED / PASS`。
...
- Status is `DESIGN FROZEN / IN PROGRESS`: Slice 1 DB foundation is `CLOSED / PASS`; Slice 2 Overview / Ownership passed local automation and user-reported desktop authenticated acceptance, including the completed ModuleHub session availability and three-module visibility recheck, with Vercel deployment verification and post-deployment mobile/PWA acceptance pending; Slices 3–4 are not started. The canonical product and technical contract is [v0.1.15 Shared Lists Specification](./v0.1.15_SHARED_LISTS_SPEC.md). v0.1.14 and v0.1.14.1 remain `CLOSED / PASS`.
...
- The four frozen implementation slices are DB foundation, overview/ownership, detail/collaboration, and ordering/mobile acceptance. The accepted Slice 1 concurrency amendment requires a narrow `set_list_item_completed` RPC and denies direct `completed` updates, keeping all completion-group changes on the parent List lock path. Slice 1 Production rollout/postflight passed; Slice 2 local automation and desktop authenticated flows passed, including dual-account filtered Realtime DELETE + RLS/canonical reread. The ModuleHub session availability and visibility recheck passed; Vercel deployment verification and mobile/PWA acceptance remain pending, so Slice 2 is not closed. Slices 3–4 remain pending. Next Action: verify Vercel deployment, then user mobile/PWA acceptance.
