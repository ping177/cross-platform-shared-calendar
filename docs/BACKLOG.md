...
Status: Slice 1 is `CLOSED / PASS — Android final acceptance deferred`. Desktop Chrome/macOS and iPhone installed PWA Push Infrastructure validation passed. Slice 2 architecture / semantics are frozen. Slice A is closed and Slice B is `CLOSED / PASS`. Slice C1 Minimal Delivery Ledger + Atomic Claim is implemented locally and under review, with 63/63 C1 pgTAP and 161/161 database regressions passing. Slice C2 sender / Edge Function / Cron has not started, the C1 patch is not in Production, and Slice C overall remains open. Android Push lifecycle acceptance remains deferred by validation strategy.
...
2. **Reminder Persistence + Ordinary Event Delivery — Slice C1 implemented / under review:** Slice A timezone/due primitives are closed. Slice B persistence + Event mutation/UI, local ordered-upgrade, database/code verification, Production database migration/postflight, frontend deployment, and Production human acceptance are complete. C1 now provides the durable server-only delivery ledger and atomic ordinary-Event claim with current-state revalidation, exact marker matching, current membership/subscription checks, due bounds, grace, and idempotency. C1 is local-only pending review. C2 current-recipient candidate scanning, Edge sender, Push delivery result transitions, Cron, secrets, and Production rollout have not started; Slice C overall is not closed.
