...
- Narrow compatibility exception: if the new authenticated frontend's first run changes persistent Production state in a way that makes the currently served frontend incompatible, real-account acceptance against the local/Preview frontend must wait until the compatible frontend is deployed. Complete the environment alignment gate, local automated/static verification, and code review first; obtain explicit Git/deployment authorization; commit/push/deploy the compatible frontend; verify the unauthenticated/static Production assets and that the old frontend is no longer the active entry point; have users close/refresh old tabs and restart PWA runtimes; then have the user perform authenticated acceptance. This is not a general deploy-before-test rule. For v0.1.10 Slice 2, `ensure_personal_space()` can add a second membership that the v0.1.9 `.limit(1).maybeSingle()` frontend cannot handle; after that creation, v0.1.9 is not a safe rollback target for the affected account.
